...
namespace parser {
    ...
    bool Grammar::isTerminal(Symbol s) const {
        return terminalFlag_[s];
    }
    bool Grammar::isNonTerminal(Symbol s) const {
        return !terminalFlag_[s];
    }
    const std::string& Grammar::name(Symbol s) const {
        return names_[s];
    }
    ...
    // These mirror the 84 rules from grammar
    ...
    void Grammar::buildProductions() {
        // helper lambdas to create symbols by name
        auto N = [&](const std::string& nm) {
            return intern(nm, false);
        };
        auto T = [&](const std::string& nm) {
            return interned_.at(nm);
        };
    ...
    void Grammar::computeNullable() {
        ...
        while (changed) {
            ...
            for (const auto& p : productions_) {
                ...
                for (Symbol s : p.rhs) {
                    if (isTerminal(s) || !nullable_[s]) {
                        allNullable = false; break;
                    }
                }
                if (allNullable) {
                    nullable_[p.lhs] = 1; changed = true;
                }
            }
        }
    }
    ...
    Grammar::firstOfSequence(const std::vector<Symbol>& seq, size_t from) const {
        ...
        if (allNullable) result.insert(epsilon_);
        return result;
    }
