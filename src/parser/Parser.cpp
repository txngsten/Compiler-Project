...
namespace parser {
    ...
    // Panic-mode synchronization set: statement / declaration boundaries. After an
    ...
    ParseResult Parser::parse(const TokenStream& stream) {
        ...
        // States run in lock-step with the parse-node stack.
        ...
        // Map a Token to its grammar terminal id.
        ...
        int safety = 0;
        const int kSafetyLimit = 2'000'000; // guard against huge loops (malicious input)
        ...
        while (ip < toks.size()) {
            ...
            if (act.type == ActionType::Shift) {
                ...
                // Error: graceful report + panic-mode recovery
                ...
                // Step 2 first: scan forward to a synchronizing terminal.
                while (ip < toks.size()) {
    ...
    // Build the tree ASCII art style
